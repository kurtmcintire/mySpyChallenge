...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    self.dataController = [[DataController alloc] initWithCompletion:nil];
    
    // Inject the DataController and PhotoController into each tab's view controller.
    UITabBarController *tabBarController = (UITabBarController *)self.window.rootViewController;
    for (UIViewController *controller in tabBarController.viewControllers) {
        if ([controller isKindOfClass:[UINavigationController class]]) {
            UINavigationController *navController = (UINavigationController *)controller;
            [self injectPropertiesInController:[navController.viewControllers firstObject]];
        }
        else {
